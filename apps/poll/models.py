...
class Poll(models.Model):
    ...
    type = models.IntegerField(default=0, choices=POLL_TYPES)
    ...
    def set_vote(self, **kwargs):
        ...
        try:
            ...
            if self.choice_required:
                raise PollVoteException(message='Invalid choice', error_code=1)
            else:
                Vote.objects.create(**kwargs)
...
class Vote(models.Model):
    poll = models.ForeignKey(Poll, verbose_name=_('poll'), related_name='votes')
    item = models.ForeignKey(Item, verbose_name=_('voted item'), related_name='votes')
    ip = models.GenericIPAddressField(verbose_name=_('user\'s IP'), blank=True, null=True)
